...
class Rover2WebDashboard(Node):
    """Web dashboard server for Rover2 monitoring and person-following control"""

    def __init__(self):
        super().__init__('rover2_web_dashboard')
        ...
        self.person_follower_clients = {
            'detection_enable': self.create_client(Trigger, '/person_follower/detection_enable'),
            'detection_disable': self.create_client(Trigger, '/person_follower/detection_disable'),
            'enable': self.create_client(Trigger, '/person_follower/enable'),
            'disable': self.create_client(Trigger, '/person_follower/disable'),
        ...
        self.person_follower_sub = self.create_subscription(
            ...
            self.person_follower_status_callback,
            10
        )
        ...
        self.get_logger().info(f"Rover2 Web Dashboard started. Package dir: {self.package_dir}")
    ...
    def broadcast_data(self):
        ...
        with self.health_lock:
            ...
            payload['person_follower_running'] = self.is_follower_running('person')
    ...
    async def handle_command(self, cmd):
        ...
        # Person Follower commands
        elif action == 'person_detection_enable':
            result = await self.call_person_follower_service('detection_enable')
            ...
            if result.get('success'):
                self.person_detection_enabled = True  # Following auto-enables detection
            return result
        elif action == 'person_follow_disable':
            return await self.call_person_follower_service('disable')
    ...
    async def call_person_follower_service(self, service_name):
        """Call a person follower service and return the result"""
        client = self.person_follower_clients.get(service_name)
        if not client:
            return {'success': False, 'message': f'Service {service_name} not found'}

        if not client.wait_for_service(timeout_sec=1.0):
            return {'success': False, 'message': f'Person follower service {service_name} not available'}
    ...
    async def call_trigger_service(self, service_name):
        ...
        # Patrol functionality removed from rover2
        client = None
        if not client:
            return {'success': False, 'message': f'Service {service_name} not found'}
...
def main(args=None):
    ...
    dashboard.get_logger().info("Rover2 Web Dashboard ready at http://<rover-ip>:8080")
