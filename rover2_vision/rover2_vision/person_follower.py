...
ROVER2 DESIGN:
  - Personal pack robot that follows you using computer vision
  - Uses person bounding box tracking to maintain following distance
  - Camera mounted for outdoor human tracking
  - Simple load/unload workflow - rover follows where you go
...
Services:
  - /person_follower/detection_enable (std_srvs/Trigger) - turn on detection/visualization
  - /person_follower/detection_disable (std_srvs/Trigger) - turn off detection (fails if following)
  - /person_follower/enable (std_srvs/Trigger) - start following (auto-enables detection)
  - /person_follower/disable (std_srvs/Trigger) - stop following (keeps detection on)
  - /person_follower/reset (std_srvs/Trigger) - reset all state to idle
"""
...
class PersonFollower(Node):
    """ROS 2 node for person detection and following using Hailo-8L - Rover2 personal pack robot."""
    ...
    def __init__(self):
        ...
        self.annotated_image_pub = self.create_publisher(
            Image, '/person_follower/annotated_image', qos_profile_sensor_data
        )
        ...
        self.enable_srv = self.create_service(
            Trigger, '/person_follower/enable',
            self.enable_callback,
            callback_group=self.service_cb_group
        )
        self.disable_srv = self.create_service(
            Trigger, '/person_follower/disable',
            self.disable_callback,
            callback_group=self.service_cb_group
        )
        self.reset_srv = self.create_service(
            Trigger, '/person_follower/reset',
            self.reset_callback,
            callback_group=self.service_cb_group
        )
        ...
        self.get_logger().info('Person Follower node initialized (Rover2)')
    ...
    def enable_callback(self, request, response):
        ...
        if self.following_enabled:
            response.success = False
            response.message = 'Person follower already enabled'
            return response
    ...
    def disable_callback(self, request, response):
        ...
        response.success = True
        if was_enabled:
            response.message = 'Person follower disabled (detection still active)'
            self.get_logger().info('Person follower DISABLED')
        else:
            response.message = 'Person follower was not enabled'

        return response

    def reset_callback(self, request, response):
        """Reset all person follower state to clean idle. Use after teleop override."""
        # Stop any motion
        self.stop_robot()
        ...
        response.success = True
        response.message = 'Person follower reset to idle - all state cleared'
        self.get_logger().info('Person follower RESET - all state cleared')
        return response
    ...
    def cmd_vel_callback(self, msg: Twist):
        ...
        If we receive a cmd_vel that we didn't publish, and person follower is enabled,
